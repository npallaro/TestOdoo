...
class AccountMove(models.Model):
    ...
    def action_add_sdi_rounding_line(self):
        ...
        # Raggruppa le righe Odoo per aliquota IVA
        odoo_by_tax = {}
        for line in self.invoice_line_ids:
            if line.display_type in ('line_section', 'line_note'):
                continue
            
            # Trova l'aliquota IVA della riga
            tax_rate = 0.0
            if line.tax_ids:
                # Prendi la prima tassa (assumiamo una sola IVA per riga)
                tax = line.tax_ids[0]
                tax_rate = tax.amount
        ...
        # Calcola IVA per ogni aliquota
        for tax_rate in odoo_by_tax:
            odoo_by_tax[tax_rate]['tax'] = odoo_by_tax[tax_rate]['untaxed'] * (tax_rate / 100.0)
        
        _logger.info(f"Odoo per aliquota: {odoo_by_tax}")
        ...
        for riepilogo in riepiloghi_xml:
            ...
            odoo_untaxed = odoo_by_tax.get(aliquota, {}).get('untaxed', 0.0)
            ...
            if abs(diff_untaxed) >= 0.01:
                ...
                _logger.info(f"Creo riga arrotondamento IVA {aliquota}%: {diff_untaxed} €")
